 ...
 */
 ...
 describe('adding, deleting or editing li items to the ul in the DOM', () => {
   ...
   describe('updating items on the list', () => {
     test('change Hello to Hell on the list', () => {
       taskClass.addTask('Hello');
       taskClass.displayTasks(itemsList);
       const list = document.querySelectorAll('#list li');
 
       taskClass.tasks[0].description = 'Hell';
       taskClass.displayTasks(itemsList);
       expect(list[0].textContent).toMatch('Hell');
     });
 
     test('change World to Metaverse on the list', () => {
       taskClass.addTask('World');
       taskClass.displayTasks(itemsList);
       const list = document.querySelectorAll('#list li');
 
       taskClass.tasks[1].description = 'Metaverse';
       taskClass.displayTasks(itemsList);
       expect(list[1].textContent).toMatch('Metaverse');
     });
   });
 });
